...
from GenomeTag.models import Annotation, Peptide, Tag, CustomUser
import loader as ld
import parser as pr
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for file in os.listdir(options["path"][0]):
            if "_cds.fa" in file:
                ...
                for chr in m[1]:
                    ...
                    os.system(
                        "samtools faidx ./projet_web/static/data/"
                        + chr.genome.id
                        + "--"
                        + chr.accession_number
                        + ".fa -o ./projet_web/static/data/"
                        + chr.genome.id
                        + "--"
                        + chr.accession_number
                        + ".fai"
        ...
        c1 = CustomUser(
            ...
            is_active=True,
            phone="+33798765432",
            affiliation="GenomeTag",
            username="Reviewer_User",
            email="reviewer@genometag.com",
        )
        c2 = CustomUser(
            role="a",
            is_active=True,
        ...
        self.stdout.write(
            "Viewer\n email= viewer@genometag.com username= Viewer_User password= example",
            ending="\n",
        )
        self.stdout.write(
            "Annotator\n email= annotator@genometag.com username= Annotator_User password= example",
            ending="\n",
        )
        self.stdout.write(
            "Reviewer\n email= reviewer@genometag.com username= Reviewer_User password= example",
            ending="\n",
        )
        ...
        with transaction.atomic():
            ...
            t3 = Tag(
                tag_id="IGORF",
                text="Ancestrally non coding sequence that started to be synthetise in peptide after evolution mecanisme",
            )
            t4 = Tag(
                tag_id="Promoter",
                text="This tag is used to represent non coding sequence on the genome, more specificaly promoter of cds",
            )
            t5 = Tag(
                tag_id="TFactor",
                text="Transcription Factor are used to allow transcription of their corresponding cds",
            )
            t1.save()
            t2.save()
            t3.save()
