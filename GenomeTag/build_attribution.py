...
def create_manual_attr(form):
    ...
    for i in range(len(form["Strand"])):
        ...
        pos = Position.objects.filter(start=start, end=end, strand=strand, chromosome=chr)
    ...
    for line in file_content:
        ...
        if not annotator.has_perm("GenomeTag.annotate"):
            return (
                "Line "
                + str(i)
                + ": The user you are trying to attribute the annotation doesn't have a role allowing him to."
            )
        ...
        for pos in range(1, len(sp), 5):
            ...
            if strand not in ["+", "-"] or not chr.exists() or not start.isdigit() or not end.isdigit():
                return (
                    ...
                    + ": The position "
                    + str(pos + 1)
                    + " has incoherent field or the chromosome doesn't exist"
                )
            start = int(start)
            end = int(end)
            chr = chr[0]
