...
def check_query(form):
    ...
    query = res + " = "
    if (
        len(form["condition"]) < 1
        or len(form["condition"]) != len(form["negation"])
        or len(form["condition"]) != len(form["text_field"])
        or (len(form["condition"]) > 1 and len(form["connector"]) != len(form["condition"]) - 1)
    ):
        return (-1, 2)
    valid_cond = 0
    for i in range(len(form["condition"])):
        ...
        if i > 0 and form["connector"][i - 1] not in ["0", "1"]:
            return (3, i)
        if is_int[int(form["condition"][i])] and not (
            form["text_field"][i].isdigit()
            or (
                len(form["text_field"][i]) > 1
...
def build_query(form):
    ...
    for i in range(len(form["text_field"])):
        if form["text_field"][i] == "":
            continue
        dic = {
            "negation": int(form["negation"][i]),
            "condition": int(form["condition"][i]),
            "value": form["text_field"][i],
        }
